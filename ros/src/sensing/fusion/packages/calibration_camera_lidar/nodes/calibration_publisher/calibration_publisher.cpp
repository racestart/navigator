...
#include "navigator_msgs/ProjectionMatrix.h"
...
static navigator_msgs::ProjectionMatrix extrinsic_matrix_msg_;
...
void tfRegistration(const cv::Mat &camExtMat, const ros::Time &timeStamp) {
  ...
  transform.setOrigin(tf::Vector3(camExtMat.at<double>(0, 3), camExtMat.at<double>(1, 3),
                                  camExtMat.at<double>(2, 3)));
  transform.setRotation(quaternion);
  broadcaster.sendTransform(tf::StampedTransform(transform, timeStamp, target_frame_,
                                                 camera_frame_));
}
...
int main(int argc, char *argv[]) {
  ...
  private_nh.param<std::string>("projection_matrix_topic", projection_matrix_topic,
                                "/projection_matrix");
  ...
  projection_matrix_pub = n.advertise<navigator_msgs::ProjectionMatrix>(projection_matrix_topic,
                                                                        10, true);
  ros::spin();
  return 0;
}
