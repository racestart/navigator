...
static void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg) {
  ...
  for(int i = 0; i < (int)scan_msg->ranges.size(); i++) {
    g_3d_scan.x[i] =
        scan_msg->ranges[i] * sin(scan_msg->angle_min + scan_msg->angle_increment * i);
    g_3d_scan.y[i] = 0;
    g_3d_scan.z[i] =
        scan_msg->ranges[i] * cos(scan_msg->angle_min + scan_msg->angle_increment * i);
  }
  ...
  plot_center_pt_line(image_lrf, center_pt, chess_size, pat_col,
                      paper_width_margin, window_scan_width, window_scan_height, scale);
